...
ATCA_STATUS hal_i2c_init(void *hal, ATCAIfaceCfg *cfg)
{
    ...
    if (bus >= 0 && bus < MAX_I2C_BUSES)
    {
        if (i2c_hal_data[bus] == NULL)
        {
            ...
            conf.clk_stretch_tick = 300; //cfg->atcai2c.baud;
            

           ESP_LOGI(TAG, "Configuring I2C");
           
            rc = i2c_driver_install(i2c_hal_data[bus]->id, I2C_MODE_MASTER);
           ESP_LOGD(TAG, "I2C Driver Install; %s", esp_err_to_name(rc));
            rc = i2c_param_config(i2c_hal_data[bus]->id, &conf);
           ESP_LOGD(TAG, "I2C Param Config: %s", esp_err_to_name(rc));
            i2c_hal_data[bus]->bus_index = bus;
            // rc = i2c_set_pin(I2C_NUM_0, 14, 2, 1, 1, I2C_MODE_MASTER);
            ESP_LOGD(TAG, "I2C Driver Set Pin: %s", esp_err_to_name(rc));
        }
        else
        {
...
ATCA_STATUS hal_i2c_send(ATCAIface iface, uint8_t *txdata, int txlength)
{
   ...
   ESP_LOG_BUFFER_HEXDUMP(TAG, txdata, txlength, 3);
    ...
    (void)i2c_master_write_byte(cmd, cfg->atcai2c.slave_address << 1 | I2C_MASTER_WRITE, ACK_CHECK_EN);
...
ATCA_STATUS hal_i2c_receive(ATCAIface iface, uint8_t *rxdata, uint16_t *rxlength)
{
    ...
    (void)i2c_master_start(cmd);
    (void)i2c_master_write_byte(cmd, cfg->atcai2c.slave_address << 1 | I2C_MASTER_READ, ACK_CHECK_EN);
